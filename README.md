...
- **Live camera** with pinch-to-zoom (optical telephoto where the phone has one, digital otherwise) and a live zoom readout, tap-to-focus, auto flash, and a close button. Zoom is also exposed as a VoiceOver/Switch Control adjustable element, and a **photo-library picker** beside the camera button routes an existing photo through the same downsample → OCR → normalize pipeline (the safe road-trip flow: passenger snaps the sign, you identify it at the next stop)
...
| Maps | MapKit (preview, Look Around) + URL schemes (Google Maps, Waze, Apple Maps) |
...
| `ContentView.swift` | Scan tab — camera button + photo-library picker (shared ingest pipeline), text field, result card, alternatives list, recent finds card |
...
| `Components.swift` | Shared UI used by more than one tab: `LandmarkDisplayMode` + `DisplayModeSegmentedPicker` (the List/Map switcher), `LandmarkThumbnail` (the one square article-image/captured-photo/signpost-placeholder thumbnail every row + map card renders), `formatLandmarkDistance` / `formatRadius` (locale-aware distance + radius strings), `announceForAccessibility`, and the shared review-prompt policy |
| `NearbyViews.swift` | Presentation subviews extracted from `NearMeView` — cold-start loading view, `NearbyRow`, `NearbyMapView` (with programmatic-camera-move filtering so the auto-fit never reads as a user pan), zoom/radius controls, selected-pin callout card |
...
| `CameraView.swift` | UIKit camera VC with pinch-to-zoom (optical + high-res capture), zoom readout, an adjustable VoiceOver/Switch Control zoom element, tap-to-focus, capture button, close button |
