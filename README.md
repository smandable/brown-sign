...
- **Live search filtering** — both Nearby and History have a search field under the List/Map picker. Partial-word, case-insensitive matching against the result title; the list (and map pins) shrink in real time as you type. Keyboard "Done" toolbar + scroll-to-dismiss both clear focus.
...
| File | Purpose |
| --- | --- |
| `BrownSignApp.swift` | `@main`, SwiftData container (registers `LandmarkLookup` + `HiddenLandmark`), TabView (Scan / Nearby / History) |
| `ContentView.swift` | Scan tab — camera button, text field, result card, alternatives list, recent finds card |
| `NearMeView.swift` | Nearby tab — discovery list + map view of brown-sign-worthy landmarks within 5 miles (SPARQL-filtered, see `WikidataLandmarkSearch.swift`); map supports pan-to-search so the user can explore beyond their immediate neighborhood by moving the map center; swipe-to-hide + cold-start auto-retry |
